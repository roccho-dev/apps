...
// app's genesis, one edge, one part. It is the compatibility gate for the pin.
...
//
// Written with semantic-map f3qlzl68qassibdpgvb075zvd9mr1sks by the operations
// in src/decision/correction.mjs. Regenerate only with a deliberate pin bump.
...
const HEAD = "sha256:d5d62cd19f87662f59f939cb53efce293278135538785701cedc12e9bbea2354";

test("the pinned provider still verifies a stored log holding an edge and a part", async () => {
  let verified;
  try {
  ...
  assert.equal(verified.decisions.length, 3);
...
test("a stored log holding a part still restores and projects", async () => {
